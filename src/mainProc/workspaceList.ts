...
ipcMain.on(IpcId.ToMainProc.CREATE_NEW_WORKSPACE, (e, arg) => {
  const wsInfo: CreateWorkspaceInfo = JSON.parse(arg)
...
ipcMain.on(IpcId.ToMainProc.CREATE_ADD_WORKSPACE, (e, arg) => {
  const wsInfo: AddWorkspaceInfo = JSON.parse(arg)
  BackendConnector.Workspace.add(wsInfo.dirPath)
    .then(() => {
      fetchWorkspaceList(e, null as any)
    })
    .catch(err => {
...
ipcMain.on(IpcId.ToMainProc.SET_WORKSPACE_ICON, (e, arg) => {
  const setWorkspaceIcon: SetWorkspaceIcon = JSON.parse(arg)
  BackendConnector.workspace(setWorkspaceIcon.workspaceId, ws => {
    ws.postIcon(setWorkspaceIcon.iconPath)
      .then(() => {
        fetchWorkspaceList(e, null as any)
      })
      .catch(err => {
...
ipcMain.on(IpcId.ToMainProc.UPDATE_WORKSPACE_NAME, (e, arg) => {
  const updateWorkspaceName: UpdateWorkspaceName = JSON.parse(arg)
  BackendConnector.workspace(updateWorkspaceName.workspaceId, ws => {
    ws.update(updateWorkspaceName.name)
      .then(() => {
...
ipcMain.on(IpcId.ToMainProc.SHOW_CONTEXT_MENU, (e, arg) => {
  ...
  const item_reload = {
    ...
    click: () => {
      BackendConnector.Workspace.getList().then(workspaceList => {
        const newWorkspaceList = workspaceList.map(w => {
          return {
  ...
  const item_update_workspace_name = {
    label: 'ワークスペース名変更',
    click: () => {
      showUpdateWorkspaceNameModal(e, msg.workspaceId)
    },
  }
  const item_delete_icon = {
    label: 'アイコンを削除',
    click: () => {
...
ipcMain.on(IpcId.ToMainProc.FETCH_WORKSPACE_ICON, (e, arg) => {
  const msg: FetchWorkspaceIcon = JSON.parse(arg)

  BackendConnector.workspace(msg.workspaceId, ws => {
    ws.fetchIcon()
      .then(imageBase64 => {
        const imageData: IconImageData = {
...
function fetchWorkspaceList(e: Electron.IpcMainEvent, selectWorkspaceId: string) {
  BackendConnector.Workspace.getList()
    .then(workspaceList => {
