...
test('画像タグ周りの操作', async () => {
  ...
  await expect(page.locator('#image-side-bar .info-area li:nth-child(2) .info-body')).toHaveText(
    'b7aa6249-2ae9-4974-8f95-c73962e2b1cc'
  )
  ...
  await expect(page.locator('#tag-ctrl-panel-tagsarea-id')).toContainText('桜')
  await expect(page.locator('#tag-ctrl-panel-tagsarea-id')).toContainText('富士山')
  ...
  await expect(page.locator('#tag-ctrl-panel-tagsarea-id')).toContainText('五重塔')
  await expect(page.locator('#tag-ctrl-panel-tagsarea-id')).not.toContainText('桜')
  await expect(page.locator('#tag-ctrl-panel-tagsarea-id')).not.toContainText('富士山')
